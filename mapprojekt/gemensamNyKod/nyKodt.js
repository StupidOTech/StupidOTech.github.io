...
// enLed <== där den skapas med data från en databas
    ...
    if (count < 11) {
        var linjeGrafik = new esri.Graphic(linjen, simpleLineSymbolVandring2);
    }
    
     else{
        var linjeGrafik = new esri.Graphic(linjen, simpleLineSymbolVandring4);
    }
...
function createPOI() {
    ...
    map.on("click", function (evt) {
        // Här lagrar vi de olika kordinaterna och passer dessa till en ny funktion ˆ}•Ÿê
        var mapPoint = esri.geometry.webMercatorToGeographic(evt.mapPoint);
        testfunka(mapPoint.x, mapPoint.y);
        /*/Steg 26-27*/
        /*/*STEG 31*/
    });
    // ˆ}•Ÿê testfunka sparar variablerna i globala variabler vilket behövs/inte behövs...
...
function paKarta(typ) {
    ...
    var standardMark = new esri.symbol.PictureMarkerSymbol("bilder-to-use/xx.png", 20, 20);
    var appendtoContent;
    if (typ == "Badplats") {
        standardMark = new esri.symbol.PictureMarkerSymbol("bilder-to-use/badplats.jpg", 18, 18);
        appendtoContent = "<tr><br><img src='kanotbilder/ashuvudet.jpg' width='550' height='auto'</tr>";

    } else if (typ == "Rastplats"){
        standardMark = new esri.symbol.PictureMarkerSymbol("bilder-to-use/Rastplatsikon.jpg", 18, 18);
        appendtoContent = "<tr><br><img src='Rastplats/sittning.jpg' width='550' height='auto'</tr>";

    }else if (typ == "Info"){
        standardMark = new esri.symbol.PictureMarkerSymbol("bilder-to-use/infosymbol.jpg", 18, 18);
        appendtoContent = "<tr><br><img src='Info/skylt.jpg' width='550' height='auto'</tr>";
    }else if (typ == "Matplats"){
        standardMark = new esri.symbol.PictureMarkerSymbol("bilder-to-use/matplatsikon.jpg", 18, 18);
        appendtoContent = "<tr><br><img src='Mat/mat.jpg' width='550' height='auto'</tr>";
    }
    ...
    var contentString = document.getElementById('info').value//beskr;
   
    
    contentString += appendtoContent;
...
*/
   ...
   //Oklar om badplatslager fungerar, om den gör så måste man lägga till en till dropdown.
...
function hideAllE(elementet) {
    

    if(elementet.checked){
        document.getElementById("hideE").checked = true;
        allShowE();
    }else{
        document.getElementById("hideE").checked = false;
        allInvisE();
    }
...
// use image variable and combine global value.
    ...
    var foodI = new esri.symbol.PictureMarkerSymbol("bilder-to-use/matplatsikon.jpg", 18, 18);
    var restI = new esri.symbol.PictureMarkerSymbol("bilder-to-use/Rastplatsikon.jpg", 18, 18);
    var infoI = new esri.symbol.PictureMarkerSymbol("bilder-to-use/infosymbol.jpg", 18, 18);
    var standard = new esri.symbol.PictureMarkerSymbol("bilder-to-use/badplats.jpg", 18, 18);
    ...
    dojo.forEach(passedValue.intresse,
        function (newFunctionStartshere) {
            ...
            if (newFunctionStartshere.name == "Rastplats") {
                ...
                appendInformationIntoPopup += "<tr><br><img src='" + newFunctionStartshere.bild + "'width='550' height='auto'</tr>";

                newPopUpTemplete.setContent(appendInformationIntoPopup);


                rastplatsLager[ci].add(galenn);
                rastplatsLager[ci].hide();
                ...
                appendInformationIntoPopup += "<tr><br><b>Extra info:<img src='" + newFunctionStartshere.bild + "'width='550' height='auto'</b></tr>";
...
function mapLegend(){
var etappFarg = "rgb(32, 76, 99)";
var slingFarg = "rgb(255, 0, 255)";
var etappFarg2 = "rgb(25, 216, 213)";

var färglista = {Vandringsleder:'red', Slingor: slingFarg, SlingorEleverad: 'yellow', Kanotleder: 'blue'};

colorize = function(färglista) {
    var container = document.getElementById('legend');
    
    for (var key in färglista) {
        ...
        label.innerHTML = key;
        box.className = "box";
        box.style.backgroundColor = färglista[key];
