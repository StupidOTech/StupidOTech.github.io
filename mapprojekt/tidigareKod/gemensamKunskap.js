...
function laddaPOIforElevation(data) {
    dojo.forEach(data.intresse, function (zeData) {
        globalaPunktArray.push([zeData.x, zeData.y]);
        skapaGrafikLagerPunktOfIntresse(zeData.x,zeData.y,zeData.name);
        //alert(zeData.x);
        // Av någon anledningn måste den ligga här och inte utanför
    });
    //skapaGrafikLagerPunktOfIntresse();
...
function makePoi() {
    var whichStops = { url: "with_elevation_POI.json", handleAs: "json", content: {}, load: showPoi };
    /*/Steg 13\*/
    dojo.xhrGet(whichStops);
    var whichStops45 = { url: "walking-poi/Biking_walking_noelevation_POI.json", handleAs: "json", content: {}, load: showPoi };
    /*/Steg 13\*/
    dojo.xhrGet(whichStops45);
}
